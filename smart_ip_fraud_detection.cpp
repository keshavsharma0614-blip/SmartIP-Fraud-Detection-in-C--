...
vector<string> loadTrustedIPs(const string& userID) {
    ifstream file("data/known_ips.txt");
    vector<string> ips;
...
void logFraud(const string& userID, const string& reason, const string& inputIP = "", const string& txnID = "") {
    ofstream file("logs/fraud_log.txt", ios::app);
    time_t now = time(0);
    string dt = ctime(&now);
...
void logSuccess(const string& userID, const string& inputIP, const string& txnID) {
    ofstream file("logs/success_log.txt", ios::app);
    time_t now = time(0);
    string dt = ctime(&now);
...
int main() {
    ...
    cout << "===== SMART IP-BASED FRAUD DETECTION SYSTEM =====\n";
    ...
    inputIP = trim(inputIP);

    if (!isValidIP(inputIP)) {
        cout << "Invalid IP address format!\n";
        logFraud(userID, "Invalid IP Format", inputIP);
        return 1;
    }
